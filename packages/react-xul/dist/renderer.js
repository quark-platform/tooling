...
const hostConfig = {
    ...
    commitMount(instance, type, props, internalInstanceHandle) {
        for (const prop in props) {
            if (typeof props[prop] === 'function') {
                instance.addEventListener(prop.replace('on', '').toLowerCase(), props[prop]);
                continue;
            }
            let propName = prop;
    ...
    prepareUpdate(instance, type, oldPropsImut, newPropsImut, rootContainer, hostContext) {
    ...
    commitUpdate(instance, updatePayload, type, prevProps, nextProps, internalHandle) {
        for (const updated in updatePayload) {
            ...
            if (typeof newVal === 'function') {
                instance.removeEventListener(propName.replace('on', '').toLowerCase(), prevProps[updated]);
                instance.addEventListener(propName.replace('on', '').toLowerCase(), newVal);
                continue;
            }
            if (propName == 'style') {
                let style = '';
...
export function render(component, container) {
    ...
    const root = renderer.createContainer(container, 0, null, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, (error) => console.error(error), null);
